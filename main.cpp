                        ...
                        if (depth < zBuffer[index]) {
                            ...
                            switch(planeta){
                                case sol:
                                    ...
                                    break;
                                case morado:
                                    fragmentShaderf = fragmentShaderMorado(fragment);
                                    SDL_SetRenderDrawColor(renderer, fragmentShaderf.r, fragmentShaderf.g, fragmentShaderf.b, fragmentShaderf.a);
                                    break;
                                case tierra:
                                    ...
                                    break;
                                case marte:
                                    fragmentShaderf = fragmentShaderMarte(fragment);
                                    SDL_SetRenderDrawColor(renderer, fragmentShaderf.r, fragmentShaderf.g, fragmentShaderf.b, fragmentShaderf.a);
                                    break;
                                case jupiter:
...
int main(int argc, char* args[]) {
    ...
    while (!quit) {
        ...
        glm::vec3 translationMatrixMorado = calculatePosition(rotation, 1.5f);
        glm::vec3 translationMatrixTierra = calculatePosition(rotation2, 2.5f);
        glm::vec3 translationMatrixMarte = calculatePosition(rotation3, 3.5f);
        glm::vec3 translationMatrixJupiter = calculatePosition(rotation4, 4.5f);
        glm::vec3 translationMatrixNeptuno = calculatePosition(rotation5, 5.5f);
        ...
        uniform4.model = createModelMatrix(translationMatrixMorado, glm::vec3(1.0f, 1.0f, 1.0f), 0.2f);
        ...
        uniform3.model = createModelMatrix(translationMatrixMarte, glm::vec3(1.0f, 1.0f, 1.0f), 0.4f);
        uniform3.view = createViewMatrix(cameraPosition, targetPosition, upVector);
        uniform3.viewport = createViewportMatrix();
        uniform3.projection = createProjectionMatrix();

        planeta3.uniform= uniform3;
        planeta3.vertex = &vertexArray;
        planeta3.shader = marte;
